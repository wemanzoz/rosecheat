...
void Triggerbot::run(UserCmd* cmd) noexcept {
    ...
    if (!config->triggerbotConfig[weaponIndex].enabled)
        weaponIndex = getWeaponClass(activeWeapon->itemDefinitionIndex());

    if (!config->triggerbotConfig[weaponIndex].enabled)
        weaponIndex = 0;

    const auto& cfg = config->triggerbotConfig[weaponIndex];
    ...
    if (now - lastContact < config->triggerbotConfig[weaponIndex].burstTime) {
