...
void Glow::render() noexcept {
    ...
    auto& glow = config->glowConfig;

    Glow::clearCustomObjects();

    if (config->glowToggleKey.isSet()) {
        if (!config->glowToggleKey.isToggled() && !config->glowHoldKey.isDown())
            return;
    }
    else if (config->glowHoldKey.isSet() && !config->glowHoldKey.isDown()) {
        return;
    }
    ...
    for (int i = 0; i < memory->glowObjectManager->glowObjectDefinitions.size; i++) {
        ...
        auto applyPlayerGlow = [applyGlow](const std::string& name, Entity* entity) noexcept {
            const auto& cfg = config->playerGlowConfig[name];
            if (cfg.all.enabled)
...
void Glow::updateInput() noexcept {
    config->glowToggleKey.handleToggle();
}
