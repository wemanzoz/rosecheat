...
void Aimbot::run(UserCmd* cmd) noexcept {
    ...
    if (!config->aimbotConfig[weaponIndex].enabled)
        weaponIndex = weaponClass;

    if (!config->aimbotConfig[weaponIndex].enabled)
        weaponIndex = 0;

    if (!config->aimbotConfig[weaponIndex].betweenShots && activeWeapon->nextPrimaryAttack() > memory->globalVars->serverTime())
        return;

    if (!config->aimbotConfig[weaponIndex].ignoreFlash && localPlayer->isFlashed())
        return;

    if (config->aimbotOnKey && !keyPressed)
        return;

    if (config->aimbotConfig[weaponIndex].enabled && (cmd->buttons & UserCmd::IN_ATTACK || config->aimbotConfig[weaponIndex].autoShot || config->aimbotConfig[weaponIndex].aimlock) && activeWeapon->getInaccuracy() <= config->aimbotConfig[weaponIndex].maxAimInaccuracy) {
        if (config->aimbotConfig[weaponIndex].scopedOnly && activeWeapon->isSniperRifle() && !localPlayer->isScoped())
            return;

        auto bestFov = config->aimbotConfig[weaponIndex].fov;
        Vector bestTarget{};
        ...
        for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
            ...
            if (!entity || entity == localPlayer.get() || entity->isDormant() || !entity->isAlive()
                || !entity->isOtherEnemy(localPlayer.get()) && !config->aimbotConfig[weaponIndex].friendlyFire || entity->gunGameImmunity())
                continue;

            for (auto bone : { 8, 4, 3, 7, 6, 5 }) {
                const auto bonePosition = entity->getBonePosition(config->aimbotConfig[weaponIndex].bone > 1 ? 10 - config->aimbotConfig[weaponIndex].bone : bone);
                ...
                if (!config->aimbotConfig[weaponIndex].ignoreSmoke && memory->lineGoesThroughSmoke(localPlayerEyePosition, bonePosition, 1))
                    continue;

                if (!entity->isVisible(bonePosition) && (config->aimbotConfig[weaponIndex].visibleOnly || !canScan(entity, bonePosition, activeWeapon->getWeaponData(), config->aimbotConfig[weaponIndex].killshot ? entity->health() : config->aimbotConfig[weaponIndex].minDamage, config->aimbotConfig[weaponIndex].friendlyFire)))
                    continue;
                ...
                if (config->aimbotConfig[weaponIndex].bone)
                    break;
            }
        }
        ...
        if (bestTarget.notNull()) {
            ...
            if (lastCommand == cmd->commandNumber - 1 && lastAngles.notNull() && config->aimbotConfig[weaponIndex].silent)
            ...
            angle /= config->aimbotConfig[weaponIndex].smooth;
            cmd->viewangles += angle;
            if (!config->aimbotConfig[weaponIndex].silent)
                interfaces->engine->setViewAngles(cmd->viewangles);

            if (config->aimbotConfig[weaponIndex].autoScope && activeWeapon->nextPrimaryAttack() <= memory->globalVars->serverTime() && activeWeapon->isSniperRifle() && !localPlayer->isScoped())
                cmd->buttons |= UserCmd::IN_ATTACK2;

            if (config->aimbotConfig[weaponIndex].autoShot && activeWeapon->nextPrimaryAttack() <= memory->globalVars->serverTime() && !clamped && activeWeapon->getInaccuracy() <= config->aimbotConfig[weaponIndex].maxShotInaccuracy)
                cmd->buttons |= UserCmd::IN_ATTACK;

            if (clamped)
                cmd->buttons &= ~UserCmd::IN_ATTACK;

            if (clamped || config->aimbotConfig[weaponIndex].smooth > 1.0f) lastAngles = cmd->viewangles;
