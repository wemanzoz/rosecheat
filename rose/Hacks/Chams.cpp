...
void Chams::renderPlayer(Entity* player) noexcept {
    ...
    if (const auto activeWeapon = player->getActiveWeapon(); activeWeapon && activeWeapon->getClientClass()->classId == ClassId::C4 && activeWeapon->c4StartedArming() && std::ranges::any_of(config->chamsConfig["Planting"].materials, [](const Config::Chams::Material& mat) { return mat.enabled; })) {
        applyChams(config->chamsConfig["Planting"].materials, health);
    }
    else if (player->isDefusing() && std::ranges::any_of(config->chamsConfig["Defusing"].materials, [](const Config::Chams::Material& mat) { return mat.enabled; })) {
        applyChams(config->chamsConfig["Defusing"].materials, health);
    }
    else if (player == localPlayer.get()) {
        applyChams(config->chamsConfig["Local player"].materials, health);
    }
    else if (localPlayer->isOtherEnemy(player)) {
        applyChams(config->chamsConfig["Enemies"].materials, health);
        ...
        if (records && !records->empty() && Backtrack::valid(records->front().simulationTime)) {
            ...
            applyChams(config->chamsConfig["Backtrack"].materials, health, records->back().matrix);
            interfaces->studioRender->forcedMaterialOverride(nullptr);
        }
    }
    else {
        applyChams(config->chamsConfig["Allies"].materials, health);
    }
}
...
void Chams::renderWeapons() noexcept {
    ...
    applyChams(config->chamsConfig["Weapons"].materials, localPlayer->health());
}
...
void Chams::renderHands() noexcept {
    ...
    applyChams(config->chamsConfig["Hands"].materials, localPlayer->health());
}
...
void Chams::renderSleeves() noexcept {
    ...
    applyChams(config->chamsConfig["Sleeves"].materials, localPlayer->health());
}
