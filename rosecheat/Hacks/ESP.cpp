...
static void renderWeaponEsp(const WeaponData& weaponData, const Weapon& parentConfig, const Weapon& itemConfig) noexcept {
    const auto& config = itemConfig.enabled ? itemConfig : (parentConfig.enabled ? parentConfig : ::config->espConfig.weapons["All"]);
    if (config.enabled) {
...
void StreamProofESP::render() noexcept {
    if (config->espConfig.toggleKey.isSet()) {
        if (!config->espConfig.toggleKey.isToggled() && !config->espConfig.holdKey.isDown())
            return;
    }
    else if (config->espConfig.holdKey.isSet() && !config->espConfig.holdKey.isDown()) {
        return;
    }
    ...
    for (const auto& weapon : GameData::weapons())
        renderWeaponEsp(weapon, config->espConfig.weapons[weapon.group], config->espConfig.weapons[weapon.name]);

    for (const auto& entity : GameData::entities())
        renderEntityEsp(entity, config->espConfig.otherEntities, entity.name);

    for (const auto& lootCrate : GameData::lootCrates()) {
        if (lootCrate.name)
            renderEntityEsp(lootCrate, config->espConfig.lootCrates, lootCrate.name);
    }

    for (const auto& projectile : GameData::projectiles())
        renderProjectileEsp(projectile, config->espConfig.projectiles["All"], config->espConfig.projectiles[projectile.name], projectile.name);

    for (const auto& player : GameData::players()) {
        if ((player.dormant && player.fadingAlpha() == 0.0f) || !player.alive || !player.inViewFrustum)
            continue;

        auto& playerConfig = player.enemy ? config->espConfig.enemies : config->espConfig.allies;
...
void StreamProofESP::updateInput() noexcept {
    config->espConfig.toggleKey.handleToggle();
}
